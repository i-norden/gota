/*
Copyright 2016 Ian Norden
Licensed under terms of MIT license (see LICENSE)
*/

// Package gota is a port of TA-Lib (http://ta-lib.org) Technical Analysis Library
// that uses decimals (https://github.com/shopspring/decimal) instead of floats
package gota
...
func BBands(inReal []dec.Decimal, inTimePeriod int, inNbDevUp dec.Decimal, inNbDevDn dec.Decimal, inMAType MaType) ([]dec.Decimal, []dec.Decimal, []dec.Decimal) {
	...
	if inNbDevUp == inNbDevDn {
		...
		if inNbDevUp.Equal(_ints[1]) {
			...
			for i := 0; i < len(inReal); i++ {
				tempReal := tempBuffer2[i].Mul(inNbDevUp)
				tempReal2 := outRealMiddleBand[i]
				outRealUpperBand[i] = tempReal2.Add(tempReal)
				outRealLowerBand[i] = tempReal2.Sub(tempReal)
			}
		...
		for i := 0; i < len(inReal); i++ {
			...
			tempReal2 := outRealMiddleBand[i]
			outRealLowerBand[i] = tempReal2.Sub(tempReal)
			outRealUpperBand[i] = tempReal2.Add(tempReal.Mul(inNbDevUp))
		}
	} else {
		for i := 0; i < len(inReal); i++ {
			tempReal := tempBuffer2[i]
